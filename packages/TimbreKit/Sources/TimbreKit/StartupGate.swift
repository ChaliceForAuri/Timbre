/// Decides whether Timbre can start, and what to tell the user when it can't.
///
/// Extracted because the *ordering* of permission requests is a real decision
/// with a real failure mode, not glue. Timbre originally checked Accessibility
...
nonisolated enum StartupGate {
    ...
    /// `nil` when Timbre can start. Otherwise the message to show.
    static func blockingMessage(for permissions: Permissions) -> String? {
        ...
        let relaunch =
            ...
            ? "" : " Then quit and reopen Timbre — Accessibility is only read at launch."
