...
/// the user can find, and Timbre can delete it on request. See GDR-0004.
///
...
final class DictationLog {
    ...
    private let logger = Logger(subsystem: "dev.hugopretorius.Timbre", category: "capture")
    ...
    /// `~/Library/Application Support/Timbre/dictations.jsonl`
    @ObservationIgnored
    private(set) lazy var fileURL: URL = {
        let base =
            FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)
            .first ?? URL(filePath: NSHomeDirectory())
        return base.appending(path: "Timbre", directoryHint: .isDirectory)
