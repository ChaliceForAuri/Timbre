<script lang="ts">
	...
	/// The hero: one dictation, start to finish, on a loop. The raw words
	/// arrive as the transcriber would emit them — filler and all — then the
	/// cleaned sentence lands in the field. Reduced motion shows the end state.
	...
	type Phase = 'warming' | 'listening' | 'polishing' | 'inserted';
	let phase: Phase = $state('inserted');
	...
	onMount(() => {
		...
		const run = () => {
			...
			at(spoken + 1500 + clean.length * 22 + 3200, run);
		};
		run();
...
<div class="relative">
	...
	<div class="absolute -bottom-6 left-6 max-w-[calc(100%-3rem)]" class:hidden={animate && phase === 'inserted' && typed > 0}>
		{#if !animate}
	...
	<div class="absolute -top-5 -right-3 sm:-right-6">
		<KeyCap symbol="⌥" name="right" gesture="hold" />
	</div>
</div>
