...
- [ ] Version 0.3.0 in `Shared.xcconfig`; CHANGELOG written from the merged records
- [ ] `release.sh` produces a DMG with a background and an Applications alias, notarized and stapled, plus the zip
- [ ] `appcast.json` generated by `release.sh` and published with the site
- [ ] Release notes with the eval numbers: polisher 11/11, commands N/N, transcripts 11/11 — the same text still gets the same correction every time
- [ ] Tag, GitHub release with assets, download page pointing at it

## Announcement

- [ ] LinkedIn post drafted: the one-sentence claim, the three keys, the privacy proof, the link; Hugo edits and approves
- [ ] Screenshots in light and dark: the pill mid-dictation, the explanation card, Settings › Dictionary
- [ ] Backstage Decisions and University linked as the "how it was built" story
- [ ] Product Hunt listing prepared, not necessarily launched the same day
